<!-- 🔴 VULNERABILIDAD INTENCIONAL: XSS Stored - el mensaje de contacto se renderiza SIN ESCAPAR como HTML directo en el banner de abajo -->
<?php
$raw_html = '';
try {
    ...
    if ($aviso && !empty($aviso['mensaje'])) {
        $raw_html = $aviso['mensaje'];
    }
} catch (Exception $e) {}
?>
<?php if ($raw_html): ?>
<!-- RAW HTML desde la BD (XSS AQUÍ) -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 items-start">
            ...
            <div class="space-y-4 flex flex-col items-center md:items-start">
                <a href="/perunet/" class="inline-block">
                    <img src="/perunet/public/img/EMPRESA/p.png?v=3.0" alt="Logo Perunet"
                        class="h-16 w-auto drop-shadow-lg mb-2 rounded-lg bg-black p-2 border-2 border-red-700" />
                </a>
            ...
            <div class="space-y-4 flex flex-col items-center md:items-start">
                ...
                <ul class="space-y-1 text-sm">
                    <li class="flex items-center space-x-2"><i
                            class="fas fa-envelope text-red-400"></i><span>servicioalcliente@perunet.pe</span></li>
                    <li class="flex items-center space-x-2"><i
                            class="fas fa-envelope text-red-400"></i><span>chiclayo01@perunet.pe</span></li>
    ...
    <div class="border-t border-gray-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <p class="text-center text-sm text-gray-400">
                Copyright <?= date('Y') ?> © <span class="font-semibold text-red-400">Perunet</span> Todos los derechos
                reservados.
            </p>
        </div>
    </div>
